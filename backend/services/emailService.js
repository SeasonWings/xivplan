...
class EmailService {
    ...
    async sendVerificationEmail(email, code, type) {
        ...
        switch (type) {
            case 'register':
                subject = 'XIVPlan账户注册验证码';
                html = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2>欢迎注册XIVPlan</h2>
            <p>感谢您注册XIVPlan账户，请使用以下验证码完成验证：</p>
            ...
            <p>此验证码将在30分钟内有效。</p>
            <p>如果您没有注册XIVPlan账户，请忽略此邮件。</p>
            <hr style="margin: 30px 0; border: none; border-top: 1px solid #eee;" />
            <p style="color: #666; font-size: 12px;">此邮件由XIVPlan自动发送，请勿回复。</p>
          </div>
        `;
                break;
            case 'reset_password':
                subject = 'XIVPlan密码重置验证码';
                html = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2>密码重置申请</h2>
            <p>您申请了XIVPlan账户密码重置，请使用以下验证码完成操作：</p>
            <div style="text-align: center; margin: 30px 0;">
              <span style="display: inline-block; padding: 15px 30px; font-size: 24px; background-color: #f0f0f0; border-radius: 5px; letter-spacing: 5px;">
                ${code}
              </span>
        ...
        const mailOptions = {
            from: `"XIVPlan" <${emailConfig.auth.user}>`,
            to: email,
            subject: subject,
            html: html,
        };
